...
class AttentionModel(nn.Module):
    ...
    def forward(self, input,):
        ...
        returns=self.discounted_returns(cost , 0.95)
        batch_size, num_nodes, embed_dim = embeddings.size()  
        sum_costs = torch.zeros(batch_size, num_nodes, device=cost.device)
        sum_costs.scatter_add_(1, pi, cost)
        sum_returns = torch.zeros(batch_size, num_nodes, device=returns.device)
        sum_returns.scatter_add_(1, pi, returns)
        sum_ll = torch.zeros(batch_size, num_nodes, device=ll.device)
        sum_ll.scatter_add_(1, pi, ll)
        count = torch.zeros(batch_size, num_nodes, device=pi.device)
        count.scatter_add_(1, pi, torch.ones_like(pi ,dtype=count.dtype))                
        return pi , sum_ll , sum_costs  ,sum_returns , value.squeeze(-1)
