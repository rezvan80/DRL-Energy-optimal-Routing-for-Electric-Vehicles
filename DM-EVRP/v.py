...
class X(nn.Module):
    ...
    def __init__(self):
        ...
        self.Query =  nn.Linear(15, 128)
        self.Key =  nn.Linear(15, 128)
        self.Value =   nn.Linear(15, 128)
        self.x=nn.Sequential(
                nn.Linear(128,512), nn.ReLU(), nn.Linear(512, 1)
            )

    def forward(self, p , p_old):
        """
        :param input:
        :param return_pi:
        :return:
        ...
        query=self.Query(p)
        key=self.Key(p_old)
        value=self.Value(p_old)
        compatibility=torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(key.size(-1))
        h=torch.matmul(torch.softmax(compatibility, dim=-1), value)
        x=self.x(h)
        return x.mean(1).squeeze()
...
def validate(data_loader, actor, render_fn, num_nodes, charging_num, save_dir='.',
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            tour_indices, _, R ,_ , v= actor.forward(x)
        reward = R.mean().item()
...
def train(actor, baseline, optimizer, lr_scheduler, task, num_nodes, train_data_out, valid_data,
    ...
    for epoch in range(iterations):  # train epoch
        ...
        train_loader = DataLoader(train_data_out, batch_size, True, num_workers=0)
        ...
        for batch_idx, batch in enumerate(train_loader):  # train batch
            ...
            #with torch.no_grad(): 
              #tour_indices_old, tour_logp_old, R_old,_ , v_old= baseline.baseline.actor(batch)
            actor.train()
            actor.set_decode_type("sample")
            
            tour_indices, tour_logp, R ,Return, v= actor(batch)
            vt=torch.roll(v, shifts=-1, dims=1)
            #reward = torch.sum(R, dim=1)  # R[batch,sequence_len] -> reward[batch]
            s = torch.roll(v, shifts=-1, dims=1)- R/200
            #s= -R/200 -v
            ...
            #ratio = torch.exp(torch.clamp((torch.sum(tour_logp,dim=1) - torch.sum(tour_logp_old.detach() , dim=1)) , -10 , 10))
            #ratio = torch.sum(tour_logp,dim=1)
            ...
            #adv = -torch.sum(R/200,dim=1).detach() - v.detach()
            ...
            #value_loss = F.huber_loss(v, torch.sum(-R_old/200,dim=1).detach() )
            ...
            #loss = (
                    ...
                    #)
            mask = torch.arange(s.size(1), device=s.device).unsqueeze(0) >= last_idx.unsqueeze(1)
            #print(s[last_idx].shape)
            s = torch.where(mask, -R/200, s ) 

            
            #print("14" , s[0] , R[0]/200 , s2[0] , mask[0])
            #s = s.masked_fill(~mask, -R/200)
            #v[mask] =0
            #s[: , last_idx:]=-Return[:, last_idx:]
            #print("14" , -Return[0] , s[0] , -R[0]/200 , last_idx[0])
            #if epoch==0 :
              #s=-Return
            
            mask2 = torch.arange(s.size(1), device=s.device).unsqueeze(0) >= (last_idx+2).unsqueeze(1)
            #print(s, mask2)
            advantage = ((s - v)*(~mask2))
            #bl_loss=torch.mean((v - s.detach()) ** 2, dim=1).mean() 
            bl_loss=F.mse_loss((~mask2)*v, ((~mask2)*(-Return)).detach())
            ...
            rewards.append(torch.mean(torch.sum(R , dim=1).detach()).item())
