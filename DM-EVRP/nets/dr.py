...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.value_head = nn.Sequential(
                nn.Linear(self.embedding_dim, 256), nn.ReLU(), nn.Linear(256, 1)
            )
    ...
    def forward(self, input,):
        ...
        _log_p,v,v2,  pi, cost= self._inner(information, distances, slope, embeddings)
        ...
        return pi , ll  ,cost, self.discounted_returns(cost/200,1) , v.squeeze() , v2.squeeze()
    ...
    def l(self , ll):
       device = ll.device
       returns = torch.zeros_like(ll, dtype=torch.float32)

       G = torch.zeros(ll.size(0), device=device)  # batch-wise
       for t in reversed(range(ll.size(1))):  # اگر cost [batch, sequence] باشد
           G = ll[:, t] +  G
           returns[:, t] = G
    ...
    def _inner(self, information, distances, slopes, embeddings):
        ...
        for _ in range(max_steps):
            ...
            log_p , value , value2 = self._get_log_p(fixed, now_idx, mask, route_feature, embeddings)
        ...
        return torch.stack(outputs, 1),torch.stack(v, 1), torch.stack(v2 , 1) , torch.stack(tour_idx, 1) , R 
    ...
    def _get_log_p(self, fixed,  now_idx, mask, route_feature, embeddings):
        ...
        value=self.value_head(query.detach())
        value2=self.value_head2(query.detach())
