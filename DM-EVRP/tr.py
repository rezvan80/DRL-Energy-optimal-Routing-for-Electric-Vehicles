...
def validate(data_loader, actor, render_fn, num_nodes, charging_num, save_dir='.',
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        with torch.no_grad():
            tour_indices, _, R ,_ , v , _= actor.forward(x)
        reward = R.mean().item()
...
def train(actor, baseline, optimizer, lr_scheduler, task, num_nodes, train_data_out, valid_data,
    ...
    for epoch in range(iterations):  # train epoch
        ...
        start = epoch_start
        #actor.value_head2.load_state_dict(actor.value_head.state_dict())
        #print(alpha)
        baseline.baseline._update_model(actor, epoch)
        for batch_idx, batch in enumerate(train_loader):  # train batch
            ...
            tour_indices, tour_logp, R ,Return, v , q= actor(batch)
            #r=torch.roll(-Return, shifts=-1, dims=1)
            #r[: , -1]=0
            ...
            #mask2 = torch.arange(s.size(1), device=s.device).unsqueeze(0) >= (last_idx+1).unsqueeze(1)
            #print(s, mask2)
            #advantage = ((s - v)*(~mask2))
            #bl_loss=torch.mean((v - s.detach()) ** 2, dim=1).mean() 
            #bl_loss=F.mse_loss((~mask2)*v, ((~mask2)*(-Return)).detach())
            #bl_loss2=F.mse_loss((~mask2)*v2, ((~mask2)*(r)).detach())
